...
def cuboid(x, y, z):
    ...
    for i, ch in enumerate(xrow):
        t[(i, 0)] = t[(i, 1+y)] = t[(1+z+i, 2+y+z)] = '.'
    if _debug:
        print(_pr(t, x, y, z))
    ycol = ['+'] + ['{}'.format('.') for j in range(y)] + ['+']
    for j, ch in enumerate(ycol):
        t[(0, j)] = t[(x+1, j)] = t[(2+x+z, 1+z+j)] = ch
    zdepth = ['+'] + ['{}'.format('.') for k in range(z)] + ['+']
    if _debug:
        print(_pr(t, x, y, z))
...
def cuboid_n(x, y, z):
    ...
    return _pr_n(t, x, y, z)


_debug = False
if __name__ == '__main__':
    for dim in ((6, 6, 10), (3, 4, 2), (4, 2, 3)):
        print("CUBOID%r" % (dim,), cuboid(*dim), sep='\n')
        print("CUBOID%r" % (dim,), cuboid_n(*dim), sep='\n')
